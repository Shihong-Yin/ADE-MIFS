...
class reciprocal_vel_obs:
    ...
    def cal_vel(self, robot_state, nei_state_list=[], obs_cir_list=[], obs_line_list=[], mode='rvo'):
        ...
        robot_state, ns_list, oc_list, ol_list = self.preprocess(robot_state, nei_state_list, obs_cir_list,
                                                                 obs_line_list)
    ...
    def preprocess(self, robot_state, nei_state_list, obs_cir_list, obs_line_list):
        ...
        ns_list = list(filter(lambda x: 0 < reciprocal_vel_obs.distance(robot_state, x) <= self.nr + x[4], nei_state_list))
        oc_list = list(filter(lambda y: 0 < reciprocal_vel_obs.distance(robot_state, y) <= self.nr + y[4], obs_cir_list))
        ol_list = list(map(lambda z: reciprocal_vel_obs.segment_in_circle(robot_state[0], robot_state[1], self.nr, z),
                           obs_line_list))
    ...
    def vel_select(self, robot_state, vo_outside, vo_inside, nei_state_list, obs_cir_list, obs_line_list, vo_list):
        ...
        if (len(vo_outside) != 0):
            ...
            temp = min(vo_inside,
                       key=lambda v: self.penalty(v, vel_des, robot_state, nei_state_list, obs_cir_list, obs_line_list,
                                                  1))
            return temp
    ...
    def penalty(self, vel, vel_des, robot_state, nei_state_list, obs_cir_list, obs_line_list, factor):
        ...
        for obs_seg in obs_line_list:
            tc = reciprocal_vel_obs.exp_collision_segment(obs_seg, robot_state[0], robot_state[1], vel[0], vel[1],
                                                          robot_state[4])
            tc_list.append(tc)
    ...
    def between_vector(line_left_vector, line_right_vector, line_vector):
        ...
        if reciprocal_vel_obs.cross_product(line_left_vector, line_vector) <= 0 and reciprocal_vel_obs.cross_product(
                line_right_vector, line_vector) >= 0:
            return True
        else:
            return False
    ...
    def between_angle(line_left_ori, line_right_ori, line_ori):
        ...
        if reciprocal_vel_obs.wraptopi(line_ori - line_left_ori) <= 0 and reciprocal_vel_obs.wraptopi(
